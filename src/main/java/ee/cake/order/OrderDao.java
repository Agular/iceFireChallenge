...
public class OrderDao {
    ...
    public List<CakeStatistic> getCakeOrderStatistics() {
        String sql =
                "SELECT name AS name, sum(order_cake.amount) AS amount FROM cake " +
                "INNER JOIN order_cake ON cake.id = order_cake.cake_id " +
                "GROUP BY name";
        return database.query( sql, new CakeOrderStatisticMapper());
    }
    ...
    private BigDecimal findTotalOrderPrice(NewOrderJson json) {
        ...
        return cake.getPrice().multiply(new BigDecimal(json.getAmount()));
    }
    ...
    private final class CakeOrderStatisticMapper implements RowMapper<CakeStatistic> {
        @Override
        public CakeStatistic mapRow(ResultSet rs, int rowNum) throws SQLException {
            CakeStatistic cakeStatistics = new CakeStatistic();
            cakeStatistics.setName(rs.getString("name"));
            cakeStatistics.setAmount(rs.getLong("amount"));
