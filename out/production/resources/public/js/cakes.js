$(function () {
    ...
    const bindCakeActions = function () {
    ...
    const showCakes = function (cakes) {
        const cakeRowTemplate = '<tr>' +
            ...
            '<td><button data-cake-id="#{cakeId}" type="button" class="btn deactivate-btn btn-danger #{deactivateBtnClass}">Deaktiveeri</button></td>' +
            '</tr>';
        const cakeRows = $.map(cakes, function (cake) {
            return cakeRowTemplate
                ...
                .replace('#{cakeAvailability}', cake.available ? 'Saadaval' : 'Ei ole saadaval')
