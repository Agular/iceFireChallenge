$(function () {
    ...
    const validateNewCakeOrder = function (newCakeOrderJson) {
        var isValid = true;
        if (newCakeOrderJson.customerName.trim() == '') {
        ...
        if (!/^[0-9]+$/.test(newCakeOrderJson.amount)) {
    ...
    $('#new-order-btn').on('click', function (e) {
        ...
        const isValid = validateNewCakeOrder(newOrderJson);
        if(isValid){
